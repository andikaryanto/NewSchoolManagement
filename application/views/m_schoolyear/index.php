                  ...
                  <div class="row">
                    ...
                    <div class="col">
                      <div class="text-right">
                        <button type="button" rel="tooltip" class="btn btn-primary btn-round btn-fab" title="<?= lang('ui_add')?>" onclick="window.location.href='<?= base_url('mschoolyear/add');?>'">
                            ...
                            <table data-page-length="<?= $_SESSION['usersettings']['RowPerpage']?>" id = "tableClass" class="table table-striped table-no-bordered table-hover dataTable dtr-inline collapsed" cellspacing="0" width="100%" style="width: 100%;" role="grid" aria-describedby="datatables_info">
                              <thead class=" text-primary">
                                <tr role = "row">
                                  <!-- <th># </th> -->
                                  <th><?=  lang('ui_name')?></th>
                                  <th><?=  lang('ui_datestart')?></th>
                                  <th><?=  lang('ui_month')?></th>
                                  <th><?=  lang('ui_year')?></th>
                                  <th class="disabled-sorting"><?=  lang('ui_isactive')?></th>
                              ...
                              ?>
                                  <tr role = "row" id = <?= $value->Id?>>
                                    <td><?= $value->Name?></td>
                                    <td><?= get_formated_date($value->DateStart, "d-M-Y")?></td>
                                    <td><?= $value->Month?></td>
                                    <td><?= $value->Year?></td>
                                    <?php 
                                    if($value->IsActive == 1 ) {
  ...
  function dataTable(){
    ...
    var table = $('#tableClass').DataTable();
     ...
     table.on( 'click', '.edit', function () {
        ...
        var id = $tr.attr('id');
        window.location = "<?= base_url('mschoolyear/edit/');?>" + id;
     } );
     ...
     table.on( 'click', '.delete', function (e) {
        ...
        deleteData(data[0], function(result){
          if (result==true)
          {
            ...
            $.ajax({
              type : "POST",
              url : "<?= base_url('mschoolyear/delete/');?>",
              data : {id : id},
      ...
      table.on( 'click', '.activate', function (e) {
        ...
        var data = table.row($tr).data();
        var id = $tr.attr('id');
        window.location = "<?= base_url('mschoolyear/activate/');?>" + id;
     });
  }
